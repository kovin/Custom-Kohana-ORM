...
class ORM_Core {
	 ...
	 * @return  void
	 */
	public function __construct($id = NULL, $father = NULL)
	{
	 ...
	 */	
  	public function __clone(){
	 	...
	 	foreach( $this->has_many as $child_table ){
  		...
  		$this->$pkey	= NULL;
  		/*
  		 * All table columns will be set with this ORM model current values, except
	...
	public function father($father_table){
		if(in_array($father_table,$this->belongs_to) && $this->loaded){
			if(!array_key_exists($father_table,$this->fathers)){
				$this->fathers[$father_table] = $this->$father_table;
			}
			return $this->fathers[$father_table];
		}else{
			return FALSE;
		}
	}
	...
	public function children($children_table){
		if(in_array($children_table,$this->has_many) && $this->loaded){
			if(!array_key_exists($children_table, $this->children)){
				$this->children[$children_table] = $this->$children_table->as_array();
			}
			
			return $this->children[$children_table];			
		}else{
			return FALSE;
		}
	}
	 ...
	 * @return  ORM_Iterator
	 */
	public function find_all($limit = NULL, $offset = NULL, ORM $father = NULL)
	{
		...
		return $this->load_result(TRUE, $father);
	}
	 ...
	 */
	protected function load_result($array = FALSE, ORM $father = NULL)
	{
		if ($array === FALSE)
		{
			// Only fetch 1 record
		...
		if ($array === TRUE)
		{
			...
			return new ORM_Iterator($this, $result, $father);
		}
